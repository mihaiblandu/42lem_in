...
/*   Updated: 2018/07/31 11:02:29 by mblandu          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	mark_the_level(s_room *room, int i, int k)
{
	while (++k < room->setlink)
		if (room->array_of_rooms[k]->ispassed == 'N')
		{
			...
			mark_the_level(room->array_of_rooms[k], i + 1, -1);
		}
		else if (room->level < room->array_of_rooms[k]->level)
		{
			...
			room->array_of_rooms[k]->ispassed = 'Y';
			mark_the_level(room->array_of_rooms[k], room->level + 1, -1);
		}
}
